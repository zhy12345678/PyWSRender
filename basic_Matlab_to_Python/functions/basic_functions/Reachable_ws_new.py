...
def reachable_ws_new(Robot, QS, Flag, **kwargs):
    couple_flag=Flag[0]
    dynamic_flag=Flag[1]
    Count, _ = QS.shape
    ...
    if opt['indices'] is None:
        Num_Array = np.linspace(4, 11, 8)
        Cal_Index = 8
        Dex = np.zeros(Count, Cal_Index)
        # print(Num_Array, Cal_Index)
    else:
    ...
    if opt['evaluate'] == 'Off':
        ...
        for i in range(Count):
            ...
            hmmi = np.sqrt(1 / np.trace(np.linalg.pinv(H @ H.T)))
            SS = S
            J = Y[:3, :]
            Singular = np.sqrt(np.linalg.det(np.dot(J, np.transpose(J))))
            ...
            for K in range(Cal_Index):
                ...
                Result = local_evaluation(Indice_Group[K], N_DoF, Y, m, M, R[i, 0])
                print(Result)
                Dex[i, Num_Array[K]-1]=Result

    #Norminization
    _, Dex_Indice_Num = Dex.shape
    print(Dex_Indice_Num)
