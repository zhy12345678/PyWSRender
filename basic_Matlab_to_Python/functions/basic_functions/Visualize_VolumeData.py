...
def visualize_volume_data(boundary, V_left, V_right, volume_size, precision, *args,**kwargs):
    ...
    if opt['opt_visual'] == 'Seperate':
        plt.plot(transfer_left[:, 0], transfer_left[:, 1], transfer_left[:, 2], '*b')
        plt.plot(transfer_right[:, 0], transfer_right[:, 1], transfer_right[:, 2], '*r')

    elif opt['opt_visual']  == 'Bimanual':
        plt.plot(transfer_dual[:, 0], transfer_dual[:, 1], transfer_dual[:, 2], '*g')
        Dex = transfer_dual
        v = boundary_ws(Dex, 0.1, 'g')

    elif opt['opt_visual']  == 'Scatter':
        plt.scatter(transfer_left[:, 0], transfer_left[:, 1], transfer_left[:, 2],transfer_left[:, 3])
        #plt.hold(True)
        plt.scatter(transfer_right[:, 0], transfer_right[:, 1], transfer_right[:, 2], transfer_right[:, 3])
        #plt.hold(True)
        plt.colorbar()
        plt.grid(False)
        plt.axis('off')
