...
condition_numbers = []  # Array to store manipulability values
...
for item in theta1:
    ...
    for i in range(len(theta2)):
        ...
        for j in range(len(theta3)):
            ...
            eigenvalues = np.linalg.eigvals(np.dot(J, J.T))
            # Calculate manipulability
            condition_number = max(eigenvalues) / min(eigenvalues)
            condition_number_column.append(condition_number)
        condition_number_row.append(condition_number_column)
    X_values.append(X_row)
    Y_values.append(Y_row)
    Z_values.append(Z_row)
    condition_numbers.append(condition_number_row)
...
Y=Y_values
Z=Z_values
condition_numbers=condition_numbers.flatten()

print(condition_numbers)
# sc = ax.scatter(X, Y,Z, c=condition_numbers,s=5,cmap='viridis')
# fig.colorbar(sc)
# ax.set_xlabel('X')
# ax.set_ylabel('Y')
# ax.set_zlabel('Z')
...
num_columns = 1
value = -600
array = np.full((num_rows, num_columns), value)
sc = ax.scatter(X,Y,condition_numbers,c=array ,s=5,cmap='viridis')
ax.set_xlabel('X')
ax.set_ylabel('Y')
ax.set_zlabel('Z')
ax.grid(True)
